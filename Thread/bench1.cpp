...
void bench_mult_dif(Args* a, double* block1, double* block2) {
    ...
    for(int i = a->k; i < lines; i += p) {
        for(int j = 0; j < lines; ++j) {
            memcpy(block1, row + i * m * m, sizeof(double) * m * m);
            //block_mult(matrix + i * m * n + j * m * m, m, m, row + i * m * m, m, block2);
            block_mult(matrix + i * m * n + j * m * m, m, m, block1, m, block2);
            memcpy(matrix + i * m * n + j * m * m, block2, m * m * sizeof(double));
        }
    }
    //pthread_barrier_wait(a->barier);
    return;
...
void* thread_func(void* arg) {
    ...
    bench_mult_dif(a, block1, block2);
